...
import (
	...
	"github.com/go-openapi/swag/internal/swtest"
...
func TestLoadFromHTTP(t *testing.T) {
	...
	_, err = LoadFromFileOrHTTP(ts3.URL, WithBasicAuth(validUsername, invalidPassword))
	require.Error(t, err)

	// basic auth, valid credentials
	_, err = LoadFromFileOrHTTP(ts3.URL, WithBasicAuth(validUsername, validPassword))
	require.NoError(t, err)
	...
	_, err = LoadFromFileOrHTTP(ts4.URL, WithCustomHeaders(testHeaders))
	require.NoError(t, err)
}
...
func TestLoadStrategy(t *testing.T) {
	loader := func(_ string) ([]byte, error) {
		return []byte(swtest.YAMLPetStore), nil
	}
	...
	b, _ := ld("")
	assert.YAMLEq(t, swtest.YAMLPetStore, string(b))

	serv := httptest.NewServer(http.HandlerFunc(swtest.ServeYAMLPestore))
	defer serv.Close()
